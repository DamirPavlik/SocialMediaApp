...
function validate_user_registration(){
    ...
    if($_SERVER['REQUEST_METHOD'] == "POST"){
        ...
        if(strlen($first_name) < 3){
            $errors[] = "Ime ne moze da ima manje od 3 karaktera";
        }

        if(strlen($last_name) < 3){
            $errors[] = "Prezime ne moze da ima manje od 3 karaktera";
        }

        if(strlen($username) < 3){
            $errors[] = "Korisniko ime ne moze da ima manje od 3 karaktera";
        }

        if(strlen($first_name) > 20){
            $errors[] = "Korsnicko ime ne moze da ima vise od 20 karaktera";
        }

        if(email_exists($email)){
            $errors[] = "Taj email vec postoji";
        }

        if(user_exists($username)){
            $errors[] = "Korisnicko ime je vec zauzeto";
        }

        if(strlen($password) < 8){
            $errors[] = "Lozinka mora da ima vise od 8 karaktera";
        }
...
function create_user($first_name, $last_name, $username, $email, $password){
        ...
        confirm(query($sql));
        set_message("Uspesno Ste Se Registruvali! Molim Vas Login-ujte Se!");
        redirect("login.php"); 
...
function validate_user_login(){
    ...
    if($_SERVER['REQUEST_METHOD'] == "POST"){
        ...
        if(empty($email)){
            $errors[] = "Email polje ne moze da bude prazno";
        }

        if(empty($password)){
            $errors[] = "Polje za lozinku ne moze da bude prazno";
        }
        ...
        if(empty($errors)){
            if(user_login($email, $password)){
                ...
                $errors[] = 'Email ili lozinka nisu ispravni. Molimo Vas pokusajte ponovo.';
            }
        }
...
function user_profile_image_upload(){
    if($_SERVER['REQUEST_METHOD'] == "POST"){
        ...
        if($_FILES['profile_image_file']['size'] > 5000000){
            $error = "Fajl je prevelik ";
            $uploadOk = 0;
        }

        if($imageFileType != 'jpg' && $imageFileType = 'png' && $imageFileType = 'jpeg' && $imageFileType = 'gif'){
            $error = "Samo jpg, png, jpeg i gif fajlovi su dozvoljneni";
            $uploadOk = 0;
        }

        if($uploadOk = 0){
            set_message('Error tokom uploaduvanja: '. $error);
        }else{
            $sql = "UPDATE users SET profile_image='$target_file' WHERE id=$user_id";
            confirm(query($sql));
            set_message("Profilna slika uploaduvana");
...
function create_post(){
    ...
    if($_SERVER['REQUEST_METHOD'] == "POST"){
        ...
        if(strlen($post_content) > 200){
            $errors[] = "Vasa objava je previse duga";
        }
    ...
    if($result->num_rows > 0){
        while($row = $result->fetch_assoc()){
            ...
            echo "<div class='post'>
                ...
                <p>" . $row['content'] . "</p>
                <p><i>Datum: <b>" . $row['created_time'] . "</b></i></p>
